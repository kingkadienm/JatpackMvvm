...
public class OnlyIconMaterialItemView extends BaseTabItem {
    ...
    public OnlyIconMaterialItemView(@NonNull Context context, @Nullable AttributeSet attrs,
        ...
        LayoutInflater.from(context).inflate(R.layout.tabstrip_item_material_only_icon, this, true);
    ...
    public void initialization(String title, Drawable drawable, Drawable checkedDrawable,
                               boolean tintIcon, int color, int checkedColor) {
        ...
        if (mTintIcon) {
            mDefaultDrawable = ColorUtil.tinting(drawable, mDefaultColor);
            mCheckedDrawable = ColorUtil.tinting(checkedDrawable, mCheckedColor);
        } else {
        ...
        if (Build.VERSION.SDK_INT >= 21) {
            setBackground(new RippleDrawable(new ColorStateList(new int[][]{{}},
                    new int[]{0xFFFFFF & checkedColor | 0x56000000}), null, null));
        } else {
            setBackgroundResource(R.drawable.tabstrip_material_item_background);
        }
    }
    ...
    public void setSelectedDrawable(Drawable drawable) {
        if (mTintIcon) {
            mCheckedDrawable = ColorUtil.tinting(drawable, mCheckedColor);
        } else {
